...
function [popStruct] = PADS_OPS(NUM_GENS,NUM_GENS_RELEASE,...
    driveParams,orgParams,dispParams,fitnessType,releaseInd)

    CONV_EFFICIENCY = driveParams(3);   % conversion efficiency;
    % generate breeding table
    raw = genBT_tethered_homing(CONV_EFFICIENCY,CONV_EFFICIENCY);
...
%         (1-s), (1-DOMINANCE*s), 1];      % relative fitness vector    
    ...
    for i = 2:(NUM_GENS+1)
        ...
        if (fitnessType == 'EA')
            ...
            maleStageRecordMat(3,:,1) = binornd(maleStageRecordMat(3,:,1), fitnessVec_males);
            femaleStageRecordMat(3,:,1) = binornd(femaleStageRecordMat(3,:,1), fitnessVec_females);
            maleStageRecordMat(3,:,2) = binornd(maleStageRecordMat(3,:,2), fitnessVec_males);
            femaleStageRecordMat(3,:,2) = binornd(femaleStageRecordMat(3,:,2), fitnessVec_females);
            maleStageRecordMat(3,:,3) = binornd(maleStageRecordMat(3,:,3), fitnessVec_males);
            femaleStageRecordMat(3,:,3) = binornd(femaleStageRecordMat(3,:,3), fitnessVec_females);
        end

        if (fitnessType == 'LA')
            % late-acting fitness cost imposed b/w pupa and adult, i.e. row
            % 10
            maleStageRecordMat(10,:,1) = binornd(maleStageRecordMat(10,:,1), fitnessVec_males);
            femaleStageRecordMat(10,:,1) = binornd(femaleStageRecordMat(10,:,1), fitnessVec_females);
            maleStageRecordMat(10,:,2) = binornd(maleStageRecordMat(10,:,2), fitnessVec_males);
            femaleStageRecordMat(10,:,2) = binornd(femaleStageRecordMat(10,:,2), fitnessVec_females);
            maleStageRecordMat(10,:,3) = binornd(maleStageRecordMat(10,:,3), fitnessVec_males);
            femaleStageRecordMat(10,:,3) = binornd(femaleStageRecordMat(10,:,3), fitnessVec_females);
        end        
